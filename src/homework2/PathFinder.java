...
public class PathFinder<N, P extends Path<N,P>> {
    ...
    private ArrayList<Path<N, P>> start_paths;
    ...
    public Path<N, P> GetShortestPath() throws NotContainsException {
        // maps nodes -> paths
        HashMap<N, Path<N, P>> paths = new HashMap<>();
        for (Path<N, P> start : this.start_paths) {
        ...
        // start nodes
        PriorityQueue<Path<N, P>> active = new PriorityQueue<>(this.start_paths);
        ...
        while (active.size() != 0) {
            ...
            Path<N, P> queueMinPath = active.poll();
            ...
            for (N child : this.graph.ListChildren(queueMin)) {
                Path<N, P> c_path = queueMinPath.extend(child);
